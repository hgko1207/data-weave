...
| 5 | 재난문자 | `disaster` | 생활/안전 | 행정안전부 긴급재난문자 (safetydata.go.kr) | ✅ mock 출시 (실 endpoint 확정 후 연동) |

> v3.5에서 1~4번 모두 실 API 연동 완료 (로또/KAMIS/정보나루/TourAPI). 5번(재난문자)은 사용자 요청으로 추가 — 부동산 확장은 "아파트 외 관심 없음"으로 보류.
