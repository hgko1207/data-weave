...
export function BuildingDetail({
  ...
  return (
    <div className="space-y-6">
      ...
      <BuildingPriceChart trades={trades} />
  ...
  return (
    <article className="rounded-xl border border-zinc-800/80 bg-zinc-900 p-6">
      ...
      <dl className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
        {roadName ? (
          <MetaItem icon={<MapPin className="h-3.5 w-3.5" aria-hidden />} label="도로명" value={roadName} />
        ) : null}
        <MetaItem
...
function BuildingStats({ trades }: { trades: ApartmentTrade[] }) {
  const amounts = trades.map((t) => t.dealAmount);
  const avg = Math.round(amounts.reduce((a, b) => a + b, 0) / amounts.length);
  const min = Math.min(...amounts);
  const max = Math.max(...amounts);
  const pyeongs = trades
    .map((t) => t.pricePerPyeong)
    .filter((n): n is number => n != null);
  const avgPyeong =
    pyeongs.length > 0
      ? Math.round((pyeongs.reduce((a, b) => a + b, 0) / pyeongs.length) * 10) / 10
      : null;
...
function BuildingAreaGroups({ trades }: { trades: ApartmentTrade[] }) {
  ...
  return (
    <article className="rounded-xl border border-zinc-800/80 bg-zinc-900">
      ...
      <ul className="divide-y divide-zinc-800/60">
        {groups.map((g) => {
          const amounts = g.trades.map((t) => t.dealAmount).sort((a, b) => a - b);
          const avg = Math.round(amounts.reduce((a, b) => a + b, 0) / amounts.length);
          const min = amounts[0];
          ...
          return (
            ...
            >
              ...
              <p className="hidden font-mono text-xs text-zinc-400 tabular-nums md:block">
                전용 {areaRange}㎡ · {g.trades.length}건
              </p>
...
function BuildingTradeList({ trades }: { trades: ApartmentTrade[] }) {
  return (
    <article className="rounded-xl border border-zinc-800/80 bg-zinc-900">
      <header className="flex flex-wrap items-baseline justify-between gap-3 border-b border-zinc-800/80 px-6 py-4">
      ...
      {/* 컬럼 헤더 — 데스크탑만 */}
      <div className="hidden grid-cols-[40px_1fr_140px_120px] items-center gap-4 border-b border-zinc-800/80 px-6 py-2.5 text-xs font-medium uppercase tracking-wider text-zinc-400 md:grid">
        ...
        {trades.map((t, idx) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              {/* 모바일: 날짜·평당가 행 아래 인라인 */}
              <p className="mt-0.5 flex items-baseline gap-2 font-mono text-xs tabular-nums text-zinc-400 md:hidden">
...
function groupByPyeong(trades: ApartmentTrade[]): AreaGroup[] {
  ...
  for (const t of trades) {
    ...
    if (cur) {
      ...
      map.set(pyeong, { pyeong, areas: [t.area], trades: [t] });
    }
  }
